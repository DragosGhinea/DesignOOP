...
public class DiscordToUserServiceImpl implements OAuth2ToUserService {
    ...
    public User getUserFromOAuth2(String accessToken) {
        ...
        try {
            ...
            user.getLinkedProviders().stream().filter(linkedProvider -> linkedProvider.getProvider().equals(ProviderType.DISCORD)).findFirst().orElseThrow(LinkedProviderNotFound::new);
